...
public class OptionsAnimationsMenu extends Menu {
	...
	public void OnPageOpened(int page) {
		...
		if (animations.size() > 21)
			animations = animations.subList(page * 21, Math.min(((page * 21) + 21), animations.size()));

		GUILayout guiLayout = getMain().getLayoutHandler().getLayout("animations");

		Inventory gui = createInventory(45, "Options | Animations");

		ItemStack back = new ItemBuilder(XMaterial.ARROW.parseItem()).setName(Utils.translate("&aBack to options"))
				.toItemStack();
		...
		if (page > 0) {
			...
			ItemStack item = new ItemBuilder(XMaterial.ENDER_PEARL.parseMaterial(), 1).setName(Utils.translate("&aPrevious page"))
					.toItemStack();
		...
		if (getMain().getAnimationHandler().getAnimations().size() > (page + 1) * 21) {

			ItemStack item = new ItemBuilder(XMaterial.ENDER_PEARL.parseMaterial(), 1).setName(Utils.translate("&aaNext page"))
					.toItemStack();
		...
		for (AnimationSettings animation : animations) {
			...
			ItemStack item = new ItemBuilder(animation.getDisplayItem()).hideAttributes()
					.setName(Utils.translate((animation.isEnabled() ? "&a" : "&c") + "Animation " + animation.getAnimationNumber()))
					.setLore("", Utils.translate(" &7ID: &6" + animation.getId() + " "), Utils.translate(" &7Display Name: &6" + animation.getFormatedDisplayName() + " "), Utils.translate(" &7Status: " + (animation.isEnabled() ? "&aEnabled" : "&cDisabled") + " "), Utils.translate(" &7Default: " + (AnimationHandler.DEFAULT_ANIMATION.equalsIgnoreCase(animation.getId()) ? "&atrue" : "&cfalse") + " "), "", Utils.translate("&eClick to edit settings "))
					.toItemStack();
