...
public class PlayerAnimationMenu extends Menu {
	...
	public void OnPageOpened(int page) {
		...
		if (!profile.getAnimation().equalsIgnoreCase("random")) {
			AnimationSettings animationSetting = getMain().getAnimationHandler()
		...
		if (animations.size() > 14)
			animations = animations.subList(page * getPageSize(), Math.min(((page * getPageSize()) + getPageSize()), animations.size()));
	...
	public void OnMenuClosed() {
		getMain().getDatabaseHandler()
				.setPlayerAnimation(getOwner().getUniqueId(), getMain().getPlayerDataHandler()
						.getData(getOwner())
						.getAnimation());
	}
	...
	private ItemStack getAnimationItem(Player player, GUILayout guiLayout, String animation) {
		...
		if (animationSettings.isNeedPermission())
			if (getMain().getAnimationHandler().haveAnimationPermission(player, animationSettings))
				...
				else item = getItem(guiLayout, animationSettings, "Selected", item);
			else item = getItem(guiLayout, animationSettings, "Locked", XMaterial.GRAY_DYE.parseItem());
		else if (!getMain().getPlayerDataHandler().getData(player).getAnimation().equalsIgnoreCase(animation))
			item = getItem(guiLayout, animationSettings, "Unlocked", item);
		else item = getItem(guiLayout, animationSettings, "Selected", item);
	...
	private ItemStack getItem(GUILayout guiLayout, AnimationSettings animationSettings, String status, ItemStack itemStack) {
		...
		String name = guiLayout.getMessage("Items.Animation." + status + ".Name")
				.replaceAll("%animation%", animationSettings.getFormatedDisplayName());
		...
		else item = new ItemBuilder(itemStack).setName(name).setLore(lore).hideAttributes().toItemStack();
