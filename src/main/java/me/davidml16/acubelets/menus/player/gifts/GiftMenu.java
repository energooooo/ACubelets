...
public class GiftMenu extends Menu {
	...
	public void OnPageOpened(int page) {
		...
		if (cubeletTypes.size() > getPageSize())
			cubeletTypes = cubeletTypes.subList(page * getPageSize(), Math.min(((page * getPageSize()) + getPageSize()), cubeletTypes.size()));

		if (cubeletTypes.size() > 0) {

			for (CubeletType cubeletType : cubeletTypes) {

				long amount = profile.getCubelets()
						.stream()
						.filter(cubelet -> cubelet.getType().equalsIgnoreCase(cubeletType.getId()))
						.count();
				...
				ItemStack item = new ItemBuilder(cubeletType.getIcon()
						.clone()).setName(Utils.translate(guiLayout.getMessage("Items.Cubelet.Name")
						.replace("%cubelet_name%", cubeletType.getName()))).setLore(lore).toItemStack();
	...
	private List<CubeletType> getCubeletTypesAvailable(Player player) {
		...
		for (CubeletType type : getMain().getCubeletTypesHandler().getTypes().values()) {
			...
			long amount = profile.getCubelets()
					.stream()
					.filter(cubelet -> cubelet.getType().equalsIgnoreCase(type.getId()))
					.count();

			if (amount > 0) cubeletTypeList.add(type);
