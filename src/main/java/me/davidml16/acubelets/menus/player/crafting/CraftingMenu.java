...
public class CraftingMenu extends Menu {
	...
	public void OnPageOpened(int page) {
		...
		if (guiLayout.getBoolean("Items.PlayerInfo.Enabled")) {
			...
			ItemStack info = new ItemBuilder(XMaterial.matchXMaterial(guiLayout.getMessage("Items.PlayerInfo.Material"))
					.get()
					.parseItem()).toItemStack();

			if (XMaterial.PLAYER_HEAD.parseItem().equals(info)) info = SkullUtils.itemFromUUID(player.getUniqueId());

			ItemStack finalItem = new ItemBuilder(info).setName(guiLayout.getMessage("Items.PlayerInfo.Name")
		...
		if (guiLayout.getBoolean("Items.Points.Enabled")) {
			...
			for (String line : guiLayout.getMessageList("Items.Points.Lore")) {
				lorePoints.add(line.replaceAll("%points_available%", "" + getMain().getPlayerDataHandler()
						.getData(player)
						.getLootPoints()));
			}

			ItemStack points = new ItemBuilder(XMaterial.matchXMaterial(guiLayout.getMessage("Items.Points.Material"))
					.get()
					.parseItem()).setName(guiLayout.getMessage("Items.Points.Name")).setLore(lorePoints).toItemStack();
		...
		for (CraftParent craft : getMain().getCubeletCraftingHandler().getCrafts()) {
			...
			for (String line : guiLayout.getMessageList("Ingredients.Lore")) {
				...
				if (line.contains("%ingredients%")) {
					...
					for (CraftIngredient ingredient : craft.getIngrediens()) {
						...
						else status = guiLayout.getMessage("Ingredients.Status.NotAvailable");

						if (ingredient.getCraftType() == CraftType.CUBELET)
							lore.add(Utils.translate(guiLayout.getMessage("Ingredients.Ingredient.Cubelet")
									.replaceAll("%name%", Utils.removeColors(getMain().getCubeletTypesHandler()
											.getTypeBydId(ingredient.getName())
											.getName()))
			...
			ItemStack item = new ItemBuilder(cubeletType.getIcon()).setName(Utils.translate(cubeletType.getName()))
					.setLore(lore)
					.toItemStack();
			item = NBTEditor.set(item, craft.getCubeletType(), NBTEditor.CUSTOM_DATA, "cubeletType");
			item = NBTEditor.set(item, Boolean.toString(getMain().getCubeletCraftingHandler()
					.haveIngredients(player, craft)), "haveIngredients");

			if (craft.getIngrediens().size() > 0)
				if (craft.getSlot() <= (getSize() - 10)) gui.setItem(craft.getSlot(), item);
