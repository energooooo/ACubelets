...
public class DecentHologramsImpl implements HologramImplementation, Listener {
	...
	public void loadHolograms(Player p, CubeletMachine box) {
		Hologram hologram = DHAPI.createHologram(UUID.randomUUID().toString(), box.getLocation()
				.clone()
		...
		if (box.getState() == CubeletBoxState.EMPTY) {
			...
			DHAPI.moveHologram(hologram, box.getLocation()
					.clone()
			...
			DHAPI.moveHologram(hologram, box.getLocation()
					.clone()
					.add(0.5, (linesNew.size() * LINE_HEIGHT_REWARD) + (box.getBlockHeight() + 0.1875), 0.5));
		}
			...
			if (holograms.get(box).containsKey(p.getUniqueId())) {
				...
				for (String line : lines) {
					if (line.contains("%reward_icon%"))
						formattedLines.add("#ICON:" + HologramItem.fromItemStack(box.getLastReward().getIcon())
								.getContent());
			...
			if (holograms.get(box).containsKey(p.getUniqueId())) {
				...
				if (page.getLines().size() == 0) {
					...
					for (int i = 0; i < lines.size(); i++) {
						...
						else DHAPI.setHologramLine(hologram, i, item);
					}
				}
