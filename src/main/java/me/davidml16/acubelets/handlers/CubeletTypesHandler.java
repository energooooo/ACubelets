...
public class CubeletTypesHandler {
	...
	private final HashMap<String, CubeletType> types;
	private final HashMap<String, File> typesFiles;
	private final HashMap<String, YamlConfiguration> typesConfigs;
			...
			if (!Character.isDigit(id.charAt(0))) {
				if (validTypeData(config)) {
					...
					if (!config.contains("type.description")) {
						List<String> lore = Arrays.asList("&7This is the most common type of ", "&7Cubelet. Initial scans indicate ", "&7that the contents of this cubelet ", "&7will be probably basic. ");
						config.set("type.description", lore);
					}

					if (!config.contains("type.lore.opening.available")) {
						List<String> loreAvailable = Arrays.asList("&5Received: &a%received% ago", "&5Expires: &a%expires%", "", "%description%", "", "&6Click to open.");
					...
					if (!config.contains("type.key")) {
						...
						ItemStack key = new ItemBuilder(XMaterial.TRIPWIRE_HOOK.parseMaterial(), 1).addGlow()
								.setName(Utils.translate(config.getString("type.name") + "'s key"))
								.setLore(Arrays.asList(Utils.translate("&8&oGo to a Cubelets Machine to"), Utils.translate("&8&oopen this key."), "", Utils.translate("&a&lLeft-Click: &6Preview rewards"), Utils.translate("&a&lRight-Click: &6Open cubelet key")))
								.toItemStack();
					...
					if (config.contains("type.icon.texture")) {
						...
						switch (icon[0].toLowerCase()) {
							case "base64":
								cubeletType.setIcon(SkullUtils.itemFromBase64(icon[1]));
								break;
							case "uuid":
								cubeletType.setIcon(SkullUtils.itemFromUUID(UUID.fromString(icon[1])));
								break;
							case "name":
								cubeletType.setIcon(SkullUtils.itemFromName(icon[1]));
								break;
						}
					} else {
						cubeletType.setIcon(new ItemBuilder(XItemStack.deserialize(Utils.getConfigurationSection(config, "type.icon"))).setLore("")
								.toItemStack());
					}
					...
					long convertedTime;
					if (Objects.requireNonNull(config.getString("type.expiration"))
							.equalsIgnoreCase("") || Objects.requireNonNull(config.getString("type.expiration"))
							.equalsIgnoreCase("none")) convertedTime = -1;
					else convertedTime = new TimeAPI(config.getString("type.expiration")).getMilliseconds();
