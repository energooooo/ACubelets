...
public class Animation17_Snowball extends BukkitRunnable {
	...
	public Animation17_Snowball(Main main, Location center, float radius, int points, boolean small, int step) {
		...
		armorStand.setHelmet(SkullUtils.itemFromBase64("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvMWRmZDc3MjRjNjlhMDI0ZGNmYzYwYjE2ZTAwMzM0YWI1NzM4ZjRhOTJiYWZiOGZiYzc2Y2YxNTMyMmVhMDI5MyJ9fX0="));
	...
	public void run() {
		...
		UtilParticles.display(Particles.FIREWORKS_SPARK, armorStand.getLocation()
				.add(armorStand.getLocation().getDirection().normalize().multiply(-1).multiply(1))
				.add(0, 1, 0), 3);

		this.step++;
		if (step >= locations.size()) step = 0;

		if (yDif < 4) yDif += 0.095;
