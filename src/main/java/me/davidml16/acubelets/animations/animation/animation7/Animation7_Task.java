...
public class Animation7_Task extends Animation {
	...
	private final Set<Animation7_Pumpkin> pumpkins = new HashSet<>();
	...
	public void onTick(int time) {
		...
		if (time == 35) {
			...
			Animation7_Pumpkin pumpkin = new Animation7_Pumpkin(getMain(), getBoxLocation().clone()
					.add(2, 0, 2), getCubeletBox().getPlayerOpening().getUuid());
			...
			if (armorStand != null) {
				...
				if (time <= 125) armorStandLocation.add(0, 0.02, 0);
		...
		ItemStack chestplate = new ItemBuilder(XMaterial.LEATHER_CHESTPLATE.parseItem()).setLeatherArmorColor(Color.WHITE)
				.toItemStack();
	...
	public void spawnGhost(ItemStack chestplate) {
		...
		Bat bat = getCubeletBox().getLocation()
				.getWorld()
				.spawn(getCubeletBox().getLocation().clone().add(0.5, 1, 0.5), Bat.class);
		ArmorStand ghost = bat.getWorld().spawn(bat.getLocation(), ArmorStand.class);
		ghost.setSmall(true);
		ghost.setGravity(false);
		ghost.setVisible(false);
		ghost.setHelmet(SkullUtils.itemFromBase64("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNjhkMjE4MzY0MDIxOGFiMzMwYWM1NmQyYWFiN2UyOWE5NzkwYTU0NWY2OTE2MTllMzg1NzhlYTRhNjlhZTBiNiJ9fX0"));
