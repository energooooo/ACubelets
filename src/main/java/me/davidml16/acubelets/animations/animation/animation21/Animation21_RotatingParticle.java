...
public class Animation21_RotatingParticle extends BukkitRunnable {
	...
	public Animation21_RotatingParticle(Main main, Location center, double radius, int points, boolean small, int step, boolean isWhite) {
		...
		if (isWhite) armorStand.setHelmet(XMaterial.WHITE_CONCRETE.parseItem());
		else armorStand.setHelmet(XMaterial.BLACK_CONCRETE.parseItem());
	...
	public void run() {
		...
		if (isWhite) ParticleDisplay.colored(armorStand.getLocation()
				.add(armorStand.getLocation().getDirection().normalize().multiply(-1).multiply(1))
				.add(0, 1, 0), Color.WHITE, 3).spawn();
		else ParticleDisplay.colored(armorStand.getLocation()
				.add(armorStand.getLocation().getDirection().normalize().multiply(-1).multiply(1))
				.add(0, 1, 0), Color.BLACK, 3).spawn();
