...
public class Animation21_Task extends Animation {
	...
	private final Set<Animation21_RotatingParticle> rotatingParticles = new HashSet<>();
	...
	public void onTick(int time) {
		...
		if (time == 24) {
			...
			Animation21_RotatingParticle rotatingParticle1 = new Animation21_RotatingParticle(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, -1.5, 0.5), 0.1, 100, true, 0, false);
			rotatingParticle1.runTaskTimer(getMain(), 0L, 1L);
			rotatingParticles.add(rotatingParticle1);

			Animation21_RotatingParticle rotatingParticle2 = new Animation21_RotatingParticle(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, -1.5, 0.5), 0.1, 100, true, 50, true);
		...
		if (time == 45) {
			armorStand = ASSpawner.spawn(getMain(), getCubeletBox(), SkullUtils.itemFromBase64("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNzRhNWRiOGVhNzk3N2M3Zjk2YTY4ZGZlMWZhNDliNjZlYWZiZWNkMzgxMWE2YjZkM2QxNzdkZGE1Zjk0MjMxMyJ9fX0="), false, false, getCubeletBox().getLocation()
					.clone()
					.add(0.5, -1.25, 0.5));
			...
			getCubeletBox().getLocation()
					.getWorld()
					.strikeLightningEffect(getCubeletBox().getLocation().clone().add(0.5, 1, 0.5));
		}
		...
		if (time > 45 && time < 121) {
			...
			if (armorStand != null) {
				...
				if (time <= 100) armorStandLocation.add(0, 0.02, 0);
		...
		if (time == 140) Sounds.playSound(getCubeletBox().getLocation(), Sounds.MySound.LEVEL_UP, 0.5F, 1F);
	...
	public void doRewardDuplication() {
		...
		for (Animation21_RotatingParticle rotatingParticle : rotatingParticles) {
			rotatingParticle.cancel();
			getMain().getFireworkUtil()
					.spawn(rotatingParticle.getArmorStand()
							.getLocation()
							.add(0, 1, 0), FireworkEffect.Type.BURST, getColors().toArray(new Color[getColors().size()]));
			if (getMain().getAnimationHandler().getEntities().contains(rotatingParticle.getArmorStand())) {
				ArmorStand armorStand = rotatingParticle.getArmorStand();
				if (armorStand != null) armorStand.remove();
				getMain().getAnimationHandler().getEntities().remove(armorStand);
