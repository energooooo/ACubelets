...
public class Animation13_Task extends Animation {
	...
	private final Set<Animation13_Gwen> guardians = new HashSet<>();
	...
	public void onTick(int time) {
		...
		if (time == 45) {
			...
			armorStand = ASSpawner.spawn(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, -0.3, 0.5), SkullUtils.itemFromBase64("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNTExM2VlNjEwODQxZGVkMjE1YWNkMmI0Y2FhZWVmODdkZmQ2ZTNkNDc2OGU3YWI0ZTE5ZWI3NmIzZDgxMjFjZiJ9fX0="), false, false, true);
		...
		if (time == 47) {
			...
			Animation13_Gwen gwen1 = new Animation13_Gwen(getMain(), armorStand, armorStand.getLocation()
					.clone()
					.add(0, 5, 0), 3, 180, 44);
			guardians.add(gwen1);
		...
		if (time == 49) {
			...
			Animation13_Gwen gwen2 = new Animation13_Gwen(getMain(), armorStand, armorStand.getLocation()
					.clone()
					.add(0, 5, 0), 3, 180, 89);
			guardians.add(gwen2);
		...
		if (time == 51) {
			...
			Animation13_Gwen gwen3 = new Animation13_Gwen(getMain(), armorStand, armorStand.getLocation()
					.clone()
					.add(0, 5, 0), 3, 180, 134);
			guardians.add(gwen3);
		...
		if (time == 53) {
			...
			Animation13_Gwen gwen4 = new Animation13_Gwen(getMain(), armorStand, armorStand.getLocation()
					.clone()
					.add(0, 5, 0), 3, 180, 179);
			guardians.add(gwen4);
		...
		if (time == 55) {
			...
			try {
				for (Animation13_Gwen guardian : guardians) {
					...
					Sounds.playSound(getCubeletBox().getLocation(), Sounds.MySound.BEACON_ACTIVATE, 0.5F, 1F);

					new BukkitRunnable() {
						@Override
						public void run() {
							Sounds.playSound(getCubeletBox().getLocation(), Sounds.MySound.BEACON_AMBIENT, 0.5F, 1F);
						}
