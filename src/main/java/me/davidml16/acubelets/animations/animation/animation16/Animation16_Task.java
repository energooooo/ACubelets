...
public class Animation16_Task extends Animation {
	...
	private final Set<Animation16_Sheep> sheeps = new HashSet<>();
	...
	public void onTick(int time) {
		...
		if (time == 6) {
			...
			Animation16_Sheep sheep1 = new Animation16_Sheep(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, -0.5, 0.5), 3.5f, 90, 0);
			sheep1.runTaskTimer(getMain(), 0L, 1L);
			sheeps.add(sheep1);

			Animation16_Sheep sheep2 = new Animation16_Sheep(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, -0.5, 0.5), 3.5f, 90, 30);
			sheep2.runTaskTimer(getMain(), 0L, 1L);
			sheeps.add(sheep2);

			Animation16_Sheep sheep3 = new Animation16_Sheep(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, -0.5, 0.5), 3.5f, 90, 60);
		...
		if (time > 0 && time < 105) {
			...
			if (armorStand != null) {
				...
				if (time < 49) armorStandLocation.add(0, 0.021, 0);

				if (time > 40) armorStand.setHeadPose(armorStand.getHeadPose().add(0, rotSpeed, 0));
		...
		if (time == 125) Sounds.playSound(getCubeletBox().getLocation(), Sounds.MySound.LEVEL_UP, 0.5F, 1F);
	...
	public void onPreRewardReveal() {
		...
		getMain().getFireworkUtil()
				.spawn(getCubeletBox().getLocation()
						.clone()
						.add(0.5, 1.50, 0.5), FireworkEffect.Type.BALL_LARGE, getColors().toArray(new Color[getColors().size()]));
